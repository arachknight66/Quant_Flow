...
class BacktestEngine:
    ...
    def _execute_exit(
        ...
        slippage_cost = abs(
            self.portfolio.position_entry_price - (self.portfolio.position_entry_price /
            (1 + self.slippage.fixed_bps / 10_000))
        ) * self.portfolio.position_shares
        pnl_net = pnl_gross - commission - entry_commission

        bars_held = 0
        if self.portfolio.position_entry_time and exit_time:
            # Rough bar count
            bars_held = self.portfolio.bar_idx_at_entry  # Will be refined below
