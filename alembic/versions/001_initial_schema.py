...
def upgrade() -> None:
    ...
    # OHLCV — critical performance table
    op.create_table(
        "ohlcv_data",
    ...
    op.create_index(
        "ix_ohlcv_asset_interval_ts",
        "ohlcv_data",
        ["asset_id", "interval", "ts"],
    )
    op.create_unique_constraint(
        "uq_ohlcv_asset_interval_ts",
        "ohlcv_data",
        ["asset_id", "interval", "ts"],
    )
    ...
    op.create_table(
        ...
        sa.Column("prob_profit", sa.Float()),
        sa.Column("kelly_fraction", sa.Float()),
        sa.Column("suggested_allocation", sa.Float()),
        sa.Column("expected_return_lo", sa.Float()),
        sa.Column("expected_return_hi", sa.Float()),
        sa.Column("var_95", sa.Float()),
        sa.Column("sharpe_est", sa.Float()),
